...
class EntryController extends Controller
{
    ...
    public function createForm(Request $request)
    {
        ...
        $event_url = env("UAEERF_PROCESS_URL") . '/eventlist?params[SearchEventCode]=' . $raceid;
        $options = [
            'headers' => [
    ...
    public function horseRider($userId)
    {
        ...
        $horse_url = env("UAEERF_PROCESS_URL") . '/horselist?params[StableID]=' . $profile->stableid;
        if (isset($profile->stableid) && $profile->stableid == "E0000014") {
            $horse_url = env("UAEERF_PROCESS_URL") . '/horselist?params[AdminUserID]=' . $profile->userid;
        }

        $rider_url = env("UAEERF_PROCESS_URL") . '/riderlist?params[StableID]=' . $profile->stableid;
        if (isset($profile->stableid) && $profile->stableid == "E0000014") {
            $rider_url = env("UAEERF_PROCESS_URL") . '/riderlist?params[AdminUserID]=' . $profile->userid;
        }
    ...
    public function create(Request $request)
    {
        ...
        foreach ($reqData as $key => $value) {
            ...
            $entry_url = env("UAEERF_PROCESS_URL") . "/addentry?params[EventID]={$raceid}&params[HorseID]={$horseid}&params[RiderID]={$riderid}&params[UserID]=" . $profile->userid;
    ...
    public function listing(Request $request)
    {
        ...
        $api_url = env("UAEERF_PROCESS_URL") . '/entries?params[SearchEventID]=' . $request->get('raceid') . '&params[SearchUserID]=' . $profile->userid;
    ...
    public function processSubstituteEntry(Request $request)
    {
        $httpClient = new \GuzzleHttp\Client();
        $api_url = '';
        $api_url = env('EIEV_URL') . '/uaeerf/execute?action=UpdateEntry&params[EntryID]=' . $request->entrycode . '&params[EventID]=' . $request->eventcode . '&params[HorseID]=' . $request->horseID . '&params[RiderID]=' . $request->riderID . '&params[UserID]=' . $request->userID;
    ...
    public function withdrawn(Request $request)
    {
        ...
        $httpClient = new \GuzzleHttp\Client();
        $api_url = '';
        $api_url = env("UAEERF_PROCESS_URL") . '/updateentry?params[EventID]=' . $raceid . '&params[SearchEntryID]=' . $entrycode . '&params[Entrystatus]=' . $status . '&params[Remarks]=Withdrawn';
